...
class ModulesTable:
    ...
    def write_modules(conn: Connection[TupleRow]) -> None:
        ...
        for name, description, orgID in modules:
            cursor.execute("SELECT 1 FROM modules WHERE name = %s", (name,))
    ...
    def module_exists(conn: Connection[TupleRow], module_id: int) -> bool:
        cursor = conn.cursor()
        cursor.execute("SELECT 1 FROM modules WHERE moduleID = %s", (module_id,))
        return cursor.fetchone() is not None

    @staticmethod
    def module_owned_by_org(conn: Connection[TupleRow], module_id: int, org_id: int) -> bool:
        cursor = conn.cursor()
        cursor.execute("SELECT 1 FROM modules WHERE moduleID = %s AND orgID = %s", (module_id, org_id))
        return cursor.fetchone() is not None
